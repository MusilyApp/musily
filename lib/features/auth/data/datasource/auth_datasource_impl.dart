...
class AuthDatasourceImpl extends BaseDatasource implements AuthDatasource {
  ...
  Future<UserEntity> login(String email, String password) async {
    return exec<UserEntity>(() async {
      final response = await _httpAdapter.post(
        '/auth/login',
        data: {'email': email, 'password': password},
      );
      await _storeTokens(response.data['token'], response.data['refreshToken']);
  ...
  Future<void> logout() async {
    return exec<void>(
      () async {
        ...
        _libraryController.updateData(
          _libraryController.data.copyWith(loadedFavoritesHash: []),
        );
